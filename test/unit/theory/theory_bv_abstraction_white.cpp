...
class TestTheoryWhiteBvAbstractionLemmas : public TestSmtNoFinishInit
{
 protected:
   ...
   * and returns a null Node for `instance(val_x, val_s, val_t, x, s, t)`.
   */
  void checkSymbolicLemma(Kind op)
  {
    NodeManager* nm = d_nodeManager.get();
    AbstractionLemmaRegistry reg(nm);
   ...
   * and implements the 6-argument `instance(val_x, val_s, val_t, x, s, t)`.
   */
      ...
      for (const std::unique_ptr<AbstractionLemma>& lemma : reg.lemmas(op))
      {
        ...
        for (uint32_t i = 0; i < w; ++i)
        {
          ...
          for (const Node& val : {pos, neg})
          {
            ...
            Node inst = lemma->instance(val, val, t, x, s, t);
            if (inst.isNull())
            {
              continue;
            }
