...
Node AbstractionLemma::instance(CVC5_UNUSED TNode valX,
                                CVC5_UNUSED TNode valS,
                                CVC5_UNUSED TNode valT,
                                CVC5_UNUSED TNode x,
                                CVC5_UNUSED TNode s,
                                CVC5_UNUSED TNode t) const
{
...
#define LEMMA_VAL(kind)                  \
  ...
  Node Lemma<LemmaKind::kind>::instance( \
      TNode val_x, TNode val_s, TNode val_t, TNode x, TNode s, TNode t) const
...
// 1*: (=> (= x 2^i) (= t (bvshl s i)))
template <>
Node Lemma<LemmaKind::MUL_POW2>::instance(TNode val_x,
                                          CVC5_UNUSED TNode val_s,
                                          CVC5_UNUSED TNode val_t,
                                          TNode x,
                                          TNode s,
                                          TNode t) const
{
  if (!val_x.isConst()) return Node();
  const BitVector& bv = val_x.getConst<BitVector>();
  unsigned p = bv.isPow2();
  if (p == 0) return Node();
  Node shiftBy = utils::mkConst(d_nm, bv.getSize(), p - 1);
  return impl(d_nm, eq(d_nm, x, val_x), eq(d_nm, t, bvshl(d_nm, s, shiftBy)));
}

// 2*: (=> (= x -2^i) (= t (bvshl (bvneg s) i)))
template <>
Node Lemma<LemmaKind::MUL_NEG_POW2>::instance(TNode val_x,
                                              CVC5_UNUSED TNode val_s,
                                              CVC5_UNUSED TNode val_t,
                                              TNode x,
                                              TNode s,
...
template <>
Node Lemma<LemmaKind::UDIV_POW2>::instance(CVC5_UNUSED TNode val_x,
                                           TNode val_s,
                                           CVC5_UNUSED TNode val_t,
                                           TNode x,
                                           TNode s,
                                           TNode t) const
{
  if (!val_s.isConst()) return Node();
  const BitVector& bv = val_s.getConst<BitVector>();
  unsigned p = bv.isPow2();
  if (p == 0) return Node();
  Node shiftBy = utils::mkConst(d_nm, bv.getSize(), p - 1);
  return impl(d_nm, eq(d_nm, s, val_s), eq(d_nm, t, bvlshr(d_nm, x, shiftBy)));
}
...
template <>
Node Lemma<LemmaKind::UREM_POW2>::instance(CVC5_UNUSED TNode val_x,
                                           TNode val_s,
                                           CVC5_UNUSED TNode val_t,
                                           TNode x,
                                           TNode s,
                                           TNode t) const
{
  ...
  return impl(d_nm, eq(d_nm, s, val_s), eq(d_nm, t, rem));
}
