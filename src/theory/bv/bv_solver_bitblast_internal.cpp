...
Node BVSolverBitblastInternal::getValue(TNode node, bool initialize)
{
  ...
  // An abstracted term is not bit-blasted, only its abstraction constant is.
  // Report the value of the latter, i.e., the value the bit-blaster committed
  // to for this term, as BVSolverBitblast::getValue does. Note that if the
  // constant is not fully assigned, we return the null node below (for
  // initialize == false) and TheoryBV::getValue falls back to evaluating the
  // term bottom-up, which is what the model does for such a term.
  if (d_am && d_am->abstractable(node) && d_am->isAbstracted(node))
  {
    Trace("bv-abstraction") << "getValue: " << node << " -> "
                            << d_am->getAbstraction(node) << std::endl;
